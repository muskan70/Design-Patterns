...
func main() {
	...
	s1 := shapeFactory(3, EQUAL_ALL, []int{4, 4, 4})
	...
	s3 := shapeFactory(3, EQUAL_TWO_SIDES, []int{4, 4, 5})
	...
	s5 := shapeFactory(4, EQUAL_ALL_Angle_90, []int{4, 4, 4, 4})
	fmt.Println("SHAPE:", s5.GetName(), " PARAMETER:", s5.GetParameter())
	s5.GetFeatures()

	s6 := shapeFactory(4, EQUAL_ALL, []int{4, 4, 4, 4})
	fmt.Println("SHAPE:", s6.GetName(), " PARAMETER:", s6.GetParameter())
	s6.GetFeatures()

	s7 := shapeFactory(4, EQUAL_TWO_SIDES_ANGLE_90, []int{4, 4, 5, 5})
	fmt.Println("SHAPE:", s7.GetName(), " PARAMETER:", s7.GetParameter())
	s7.GetFeatures()

	s8 := shapeFactory(4, EQUAL_TWO_SIDES, []int{4, 4, 5, 5})
